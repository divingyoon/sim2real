...
def test_right_builder_matches_the_recorded_env_sample():
    ...
    from grasp_s2r_obs_builder import (
        SEGMENTS, assemble_actor_obs, hand_dof_order, normalized_joint_err,
        tip_body_order)
    ...
    # ★★`joint_err` 만 **프로필 순**이다 — env `_joint_pos_err()` 가 `_syn_ids` 로 잰다.
    #   이 표본은 목표≈실측 시점이라 DOF 순으로도 통과했었다(스크램블이 안 보인다).
    #   09.03 sim 대조에서 스텝이 갈수록 벌어져(0.0022→0.0087) 드러났다.
    ip = [names.index(n) for n in HAND_JOINT_NAMES]
    ...
    built = assemble_actor_obs(
        arm_q=q[ia], arm_qd=qd[ia], hand_q=q[ih], hand_qd=qd[ih],
        joint_err_profile_order=normalized_joint_err(q[ip], qt[ip], 1.2),
...
def test_canonical_hand_order_would_be_caught():
    ...
    """
    from grasp_s2r_obs_builder import (
        SEGMENTS, assemble_actor_obs, normalized_joint_err, tip_body_order)
    ...
    built = assemble_actor_obs(
        arm_q=q[ia], arm_qd=qd[ia], hand_q=q[ih], hand_qd=qd[ih],
        joint_err_profile_order=normalized_joint_err(q[ih], qt[ih], 1.2),
        palm_pos=bp[pi], palm_quat=bq[pi],
        tip_pos=np.array([bp[bod.index(t)] for t in tip_body_order("r")]),
        cup_pos=cup, goal_pos=goal,
        tip_force_world=np.zeros((5, 3)), tip_quat=np.tile([1.0, 0, 0, 0], (5, 1)),
