...
# ---------------------------------------------------------------------------
# velocity_limited_target — fabric_q 직접 추종(rate-limit only, 실제위치 무참조)
# 시그니처: (target, last_setpoint, max_vel, dt)
# ---------------------------------------------------------------------------
_STEP = 0.1 * (1.0 / 60.0)   # max_vel 0.1 · dt 1/60


def _vlt(target, last, max_vel=0.1, dt=1.0 / 60.0):
    return velocity_limited_target(target, last, max_vel, dt)


def test_vlt_advances_from_last_by_step():
    # 큰 목표 → 직전 세트포인트에서 step 만큼만 전진.
    out = _vlt(np.full(7, 0.5), np.zeros(7))
    assert np.allclose(out, _STEP)


def test_vlt_small_delta_reaches_target_no_overshoot():
    # 델타가 step 이하면 목표 그대로(무왜곡 통과).
    tgt = np.array([0.0005, -0.0003, 0.0])
    out = _vlt(tgt, np.zeros(3))
    assert np.allclose(out, tgt)


def test_vlt_tracks_target_ignoring_actual_no_droop():
    # ★핵심: 실제 위치를 참조하지 않으므로, 팔이 처져도 세트포인트는 target 을 향해 계속 전진.
    #   (이전 follow_err 캡은 처진 실제를 따라 내려가 홀딩 토크를 죽였음 — 이제 없음)
    last = np.zeros(3)
    out1 = _vlt(np.full(3, 1.0), last)
    out2 = _vlt(np.full(3, 1.0), out1)
    assert np.allclose(out1, _STEP)
    assert np.allclose(out2, 2 * _STEP)   # target 쪽으로 계속 전진
    assert np.all(out2 > out1)


def test_vlt_does_not_overshoot_target():
    # 세트포인트는 target 을 넘지 않는다(폭주 없음): last 가 target 근처면 target 에서 멈춤.
    tgt = np.full(3, 1.0)
    out = _vlt(tgt, np.full(3, 0.9995))   # target 까지 0.0005 < step
    assert np.allclose(out, tgt)
...
def test_vlt_bad_params_and_shape():
    with pytest.raises(ValueError):
        _vlt(np.zeros(4), np.zeros(3))          # shape 불일치
    with pytest.raises(ValueError):
        velocity_limited_target(np.zeros(3), np.zeros(3), 0.0, 1 / 60)
    with pytest.raises(ValueError):
        velocity_limited_target(np.zeros(3), np.zeros(3), 0.1, 0.0)
