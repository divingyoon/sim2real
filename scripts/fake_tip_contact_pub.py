...
발행: /dg5f_right/contact_forces  std_msgs/Float64MultiArray (5×0.0)
"""
...
class FakeTipContact(Node):
    def __init__(self, rate_hz: float) -> None:
        ...
        self.get_logger().info(
            f"임시 접촉(0) 발행: /dg5f_right/contact_forces ({NUM_TIPS}×0.0), {rate_hz:g}Hz\n"
            "  ⚠️ 실접촉 아님 — 실 dg5f joint_states 병행 stopgap(게이트 통과용)"
        )

    def _tick(self) -> None:
        msg = Float64MultiArray()
        msg.data = [0.0] * NUM_TIPS
