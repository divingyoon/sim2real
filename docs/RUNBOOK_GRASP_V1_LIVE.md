...
- ⚠️ **브리지가 떠 있는 동안 robotctl 수동 조작 불가** — 브리지가 마지막 목표를 60Hz 로 계속
  스트리밍(홀딩 설계)해 액션 goal 을 매 tick 덮는다. 수동 이동 순서: 브리지 pkill → robotctl
  → 브리지 재기동(fresh 상태는 정책 첫 명령 전까지 무발행·실측 위치서 세트포인트 시작이라 안전).
