...
## 6-3. 좌 그리퍼 그림자 — sim 쪽 실측 (2026-08-25)
...
| | mean | p95 | max |
|---|---|---|---|
| 위치 | **11.89 mm** | 49.31 | 401.97 |
| 자세 | 3.86° | 21.12 | 80.74 |

정상 구간은 **3.6 mm** 로 안정된다(스텝 200~1000 실측). 큰 값은 초기 전이와 **도달 불가
목표** 구간이다 — `probe_gripper_left_fabrics.py` 로 PALM_BOX 8꼭짓점을 재 보면 도달오차가
0.2 mm ~ **200.5 mm** 로 갈린다(먼 x·낮은 z 쪽이 나쁘다). 정책이 지시할 수 있는 박스가
팔이 갈 수 있는 곳보다 넓다 — 제어 실패가 아니라 **워크스페이스 사실**이다.

**L2 — sim 물리가 fabric 해를 따라가나** (물리 TCP vs FK): mean **3.46 mm** · p95 18.78 · max 116.55.
★TCP 는 `l_hl_gripper_base` + z 0.08 로 합성해야 한다. base 바디를 그대로 쓰면 두 점이
구조적으로 80 mm 떨어져 있어 L2 가 "추종오차 99 mm"로 읽힌다(실제로 한 번 그렇게 읽었다).

**정책이 요구하는 것** — `ACTION_CONTROL_DEMAND.md` §③ 이 ⬜ 로 남겨 둔 "요구(demand)" 쪽
...
→ **평균 요구는 실기 한계의 5 % 미만인데 첨두는 한계의 2배다.** 프로필 velocity 상한이
2.0 rad/s 이므로 1배속 재생은 거부되어야 하고, **`--rate-scale ≤ 0.51`** 이면 들어온다
(0.5 에서 1.965 rad/s). 즉 ③ 대역폭 격차의 판정은 "평균은 여유, 첨두는 초과"다 —
전 구간 감속이 아니라 첨두 구간만 문제라는 뜻이고, 대응 선택지 A~D 의 저울이 달라진다.

**중력 처짐 보상분** (액션 항이 관절공간에 더하는 적분항)
...
| l_aj_3 | 18.21 | **67.50** |
| l_aj_4 | 16.68 | **67.50** |
| l_aj_5~7 | 4.8~9.7 | **17.50** |

★ max 값 67.50 / 17.50 은 정확히 `ARM_IK_MAX_TRACKING_ERROR`(= effort/강성) 이다 —
**보상기가 상한에 포화한다**. 그 상한의 분모가 **sim 강성 400** 이라 실기 70/60/10 에서는
같은 각도가 전혀 다른 토크를 뜻한다. 그래서 기록은 `fabric_q` 와 `droop` 을 **따로** 남긴다.

### ★재현성 경고 — 이 기록이 어떤 코드로 나왔나

fab_test16/17/20 은 **커밋에 없는 코드로 학습됐다.** 학습 시점 태스크 파일은 미커밋
워킹트리 상태였고, `test_history.md` 는 "Uncommitted 변경 없음" 이라고 적었는데 **그것이
틀렸다**(로컬·arm4090 양쪽 해시로 확인). 이후 두 머신 모두 갱신되어 그 상태는 **어디에도
남아 있지 않다** — git 이력 25개 커밋을 훑어도 없다.

그래서 위 수치는 **fab_test16 체크포인트 × 08.25 오후 시점 제어기**로 잰 것이다. 그 시점
제어기는 학습 당시와 다르다(리미터 해제 · fabric 시간 2배 · damping 20→10 · 속도
피드포워드 0→1.0 — 자매 세션의 "원본 정합" 작업). 판단에 쓸 때 이 구분이 중요하다:

- **유효하다**: 팔 추종·Fabrics IK·요구 속도 — 실기가 따라갈 대상은 *지금* 제어기가 내는
  관절 목표이고, 그게 곧 배포될 것이다.
- **유효하지 않다**: 이 궤적으로 정책의 **과제 성능**을 판단하는 것. 액션 의미가 달라졌다.

npz 는 `meta_task_sha256`·`meta_fabrics` 로 **자기가 어떤 소스로 나왔는지 스스로 답한다**.
다음 런부터는 그 필드로 대조할 것.

---

## 6-4. Fabrics 를 실기에서 굴릴 수 있나 (2026-08-25)

"fabric IK 연동"은 네 조건으로 갈린다. 셋은 확인됐고 **하나가 남았다.**

| # | 조건 | 상태 | 근거 |
|---|---|---|---|
| ① | 배포 파이썬(ROS py3.10)에서 fabrics_sim 이 GPU 로 돈다 | ✅ | rclpy·torch 2.7.1+cu128·warp 1.8.1·CUDA 공존 확인 |
| ② | 자산(URDF·world·params)이 sim 학습과 같다 | ✅ | `test_the_fabrics_check_actually_runs_for_a_manager_based_task` 가 고정 |
...
| | mean | p95 | max |
|---|---|---|---|
| 관절 최대편차 | 5.11e-4 | 1.20e-3 | 1.41e-3 rad |
| **TCP 편차** | **0.016 mm** | 0.071 | **0.152 mm** |

추세는 유계다(앞 1/4 0.026 mm → 뒤 1/4 0.007 mm) — 누적 발산이 아니라 float32 적분
차이다. **Fabrics 자신의 수렴 바닥(정상구간 L1 3.6 mm)보다 20배 작고**, 실기 중력 처짐
예측(53 mm)에 비하면 무시할 크기다.

★판정을 rad 가 아니라 **TCP 밀리미터**로 한다. 처음엔 1e-4 rad 을 문턱으로 잡았다가
1.4e-3 에서 실패로 찍혔는데, 그 숫자가 무엇을 뜻하는지 아무도 답할 수 없었다. fabric
자신의 FK 로 환산하니 0.152 mm 였다 — 같은 측정, 정반대 결론.

**④ 가 남았다.** 지금 준비된 것은 **그림자 경로**다: sim 이 Fabrics 를 풀어 관절 목표를
남기고 실기는 그걸 따라간다. 실기에서 Fabrics 가 도는 것이 아니다. 라이브로 가려면:

- 좌 그리퍼용 정책 루프 노드가 필요하다. `grasp_policy_core.py` 가 템플릿이지만
  **델타+기준점 규약**(obs 154D / act 21D, tesollo right)용이고 좌 그리퍼는 **절대 palm
  7D · manager-based** 다. 규약이 정반대라 그대로 못 쓴다.
- `fabric_q` 를 매 tick 실측으로 **재동기화하지 않는** 계약을 옮겨야 한다. 재동기화하면
  느린 실팔 위치로 명령이 붕괴해 전진 불가가 된다(08.03 RUNNING 동결).
- 처짐 보상 적분항의 상한이 `effort/400`(sim 강성)이다. 실기 kp 70/60/10 에서는 같은
  각도가 다른 토크다 — 재유도하거나 꺼야 한다.
- **유휴 우팔이 world 모델의 가정과 맞아야 한다.** `open_gripper_left_boxes_no_table.yaml`
  이 우팔을 (0.25, −0.20, 0.55) 반경 0.15 의 **고정 구**로 세워 둔다. 실기 우팔이 다른
  곳이면 fabric 은 없는 장애물을 피하고 있는 팔은 피하지 않는다. → 그림자 재생기에
  `IDLE_ARM_MISMATCH_RAD = 0.15` 게이트를 넣었다(거부 시 관절명을 지목한다).
